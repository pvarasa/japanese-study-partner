...
- `backend/app/models.py` — SQLAlchemy models: Item (word/grammar/expression with SRS fields), ReadingCard (an item's kanji-reading SRS track), KanjiInfo (cached per-kanji details), Tag, Source, Setting (per-user key/value), StudySession. All carry `user_id` **except `Tag` and `KanjiInfo`**, which are globally shared (items are still user-scoped, so this leaks no data — but tag names and kanji details are common to all users). `Item.is_leech` is a Python property delegating to `srs.is_leech`; `srs.py` keeps its `models` import under `TYPE_CHECKING` so the two don't cycle
...
- `backend/app/kanji.py` — Pure helpers for the kanji-reading drill: `is_reading_candidate` (word/expression with kanji and a stored reading), `kanji_families` (other library words sharing each kanji, shown on the reveal), and `readings_in_word` (a left-to-right heuristic guessing which on/kun reading a word uses, allowing rendaku and gemination; irregular readings like 今日 match nothing and are left unmarked)
- `backend/app/kanji_details.py` — Per-kanji details for the reading drill's reveal (meaning, on/kun, unofficial JLPT level, strokes, components, mnemonic, origin). One Claude call per word covers its **uncached** kanji; results live in `KanjiInfo`, which — like `Tag` — has **no `user_id`** (kanji facts are universal, so each is paid for once). Served by `GET /api/kanji/item/{item_id}` (`routers/kanji.py`); the frontend fetches it when the card appears and prefetches the next card (`pages/study/useKanjiDetails.js`), so the ~8s first generation is usually hidden behind the guess
- `backend/app/japanese.py` — fugashi tokenizer helpers (`annotate`, `tokenize`, `reading_for`), factored out of `routers/furigana.py` so `cloze.py` doesn't have to import a router (same reasoning as `levels.py`). `routers/furigana.py` re-exports them for back-compat. The single MeCab tagger is shared across threadpool workers, so every parse goes through `tokenize`, which holds a lock and copies token fields out — don't call the tagger directly
...
- `backend/app/routers/study.py` — Due items (excludes suspended), review submission, dashboard stats, session tracking, `GET /study/history` for the retention chart, and the kanji-reading queues (`/study/reading/due|practice`)
...
- `frontend/src/api.js` — All API client functions. A `fetch` that gets no response at all becomes a readable "Network error" message
- `frontend/src/storage.js` — Safe `localStorage` access with a timestamped envelope (`loadValue`/`saveValue`, optional `maxAgeMs`). Still reads legacy bare values. `hooks/usePersistentState.js` builds on it
- `frontend/src/hooks/useOnPageVisible.js` — Runs a callback when the tab becomes visible after being hidden for at least `minHiddenMs`. Dashboard and the JLPT-level sync use it to refresh stale data
...
- `frontend/src/pages/` — Dashboard, Items (library), Study (flashcards + drills + kanji reading + example-sentence button; its hook and card components live in `pages/study/`), Reading (AI passages), Converse (Q&A with mic when enabled), Ingest (import)
...
- **Items have two SRS tracks.** The `Item.srs_*` fields are meaning recall; `ReadingCard` (one per item, created lazily on its first reading review) is the kanji-reading drill (`kanji_reading` mode, `/study/reading/due|practice`, `POST /study/review` with `card: "reading"`). Every other study mode shows furigana, which is why reading needs its own schedule. New reading cards are served best-known-meaning first. Reading cards never auto-suspend (`process_review(..., auto_suspend=False)`), and SQLite doesn't enforce FKs here, so the card is deleted with its item via the ORM cascade on `Item.reading_card`
...
- The backend serves `frontend/dist` via `SPAStaticFiles` (`app/spa.py`). Any 404 whose path isn't under `api/` and has no file extension gets `index.html`, so deep links and reloads on client routes work on port 8000. Starlette passes `get_response` an **OS-normalised path** (backslashes on Windows), so split on both separators when checking segments
- **Page state must survive a reload.** Mobile browsers discard backgrounded tabs, and the Vite dev server reloads the page when its HMR socket reconnects after the phone sleeps. So any work in progress lives in `localStorage` via `storage.js` (`loadValue`/`saveValue`, a timestamped envelope with an optional `maxAgeMs`) or `usePersistentState`, and Library filters live in the URL. `useStudySession` keeps the whole session in **one object** so it's saved and restored consistently. Its in-flight work (question fetch, sentence evaluation) is driven by effects keyed off persisted state, which means a restored card picks the pending request back up by itself. Keep new session-like state in a single snapshot rather than several keys with separate expiry, and don't persist transient `loading` flags
- `DISABLE_HMR=1` sets Vite's `server.ws: false`, which stops the reconnect reload but also stops hot updates. Plain `hmr: false` is **not** enough, because the client still connects and still reloads on reconnect
- Async continuations in page hooks must check they're still on the card or session they started from. Use `items` identity plus `current` in the updater, or a run counter for starts. Rating uses a ref guard against double taps. A sentence_build evaluation that fails surfaces an error with a retry. **Never fabricate an "incorrect" verdict**, because that schedules a lapse for what was only a dropped connection
- Converse deliberately does **not** end its session on unmount: the conversation resumes when you come back, and starting a new topic closes the old session. The unmount cleanup only releases the microphone
...
- `EXAMPLES_PER_ITEM` is a **floor the prompts request, not an invariant the schema enforces** — nothing rejects a row with one example or trims one with four. The Study card caps its reveal at `SHOWN_EXAMPLES` (`Study.jsx`) for a stable card height, picking a random subset (not just the leading slice) so a card with more stored than shown doesn't reveal the same ones every time; the backfill's top-up pass fills shortfalls. Raising the constant means re-running the backfill; the older rows aren't rewritten on their own
...
- JLPT level is persisted in the `settings` table (composite PK `user_id + key`). The frontend also caches it in `localStorage` (`jlpt-level`) for instant load, then reconciles with the server on mount and again when the tab becomes visible
